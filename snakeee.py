...
def game():
    ...
    #вес еды (сколько очков за неё дадут, делаем рандом от 1 до 3)
    food_value = random.randint(1, 3)
    #запоминаем время появления еды (для таймера)
    ...
    while running:
        ...
        #проверка таймера еды — если еда лежит слишком долго, меняем её
        now = pygame.time.get_ticks()  #текущее время в миллисекундах
        if now - food_spawn_time > 3000:  #если прошло больше 3 секунд
            food = generate_food(snake)   #новая еда
            food_value = random.randint(1, 3)  #новый вес еды
            food_spawn_time = pygame.time.get_ticks()  #снова запоминаем время появления

        #проверяем, съела ли змейка еду
        if (head_x, head_y) == food:
            #добавляем к счёту столько, сколько стоит еда
            score += food_value

            #создаём новую еду
            food = generate_food(snake)
            #новый рандомный вес еды
            food_value = random.randint(1, 3)
            #снова запоминаем время появления еды
            food_spawn_time = pygame.time.get_ticks()

            #каждые 3 очка новый уровень и ускорение (оставил как было)
            if score % 3 == 0:
                level += 1
                speed += 2
        else:
        ...
        #рисуем значение еды над ней (чтобы было видно "вес" пищи)
